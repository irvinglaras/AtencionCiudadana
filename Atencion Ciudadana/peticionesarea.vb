...
#Region "AdaptaBotones y formulario redondo"
    ...
    Private Sub peticionesarea_Paint(sender As Object, e As PaintEventArgs) Handles MyBase.Paint
        '-> Smooth Outer Border
    ...
    Private Sub peticionesarea_ResizeEnd(sender As Object, e As EventArgs) Handles MyBase.ResizeEnd
        Me.Invalidate()
    End Sub
    Private Sub peticionesarea_SizeChanged(sender As Object, e As EventArgs) Handles MyBase.SizeChanged
        Me.Invalidate()
    End Sub
    Private Sub peticionesarea_Activated(sender As Object, e As EventArgs) Handles MyBase.Activated
        Me.Invalidate()
    End Sub

#End Region

    Private Sub peticionesarea_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        sAreadID = My.Settings.sAreaID
        lblArea.Text = GetArea()
        CargaAreas()
        CargaDatos(1)
        CargaTotales
    ...
    Private Sub CargaTotales()
        lblE.Text = BuscaTotales(3)
        lblNA.Text = BuscaTotales(2)
        lblF.Text = BuscaTotales(4)
        lblC.Text = BuscaTotales(5)
    End Sub
    ...
    Private Sub CargaDatos(ByVal nOpcion As Byte)
        Try
            Dim sSQL As String = ""
            If nOpcion = 1 Then
                sSQL = "SELECT p.Folio, p.Nombre, p.Peticion, p.Telefono, p.estatus, a.area, p.fecha as FechaInicio FROM peticiones as p inner join areas as a on a.areaid=p.areaid WHERE p.areaid='" & sAreadID & "' ORDER BY folio DESC"
            Else
    ...
    Private Sub btnGuardar_Click(sender As Object, e As EventArgs) Handles btnGuardar.Click
        Try
            ...
            sSQL = "update peticiones set estatus=@estatus, fechatermino=@fechatermino, observaciones=@observaciones WHERE folio=" & txtFolio.Text & ""
            'Else
            ...
            If result > 0 Then
                MessageBox.Show("Petición actualizada exitosamente....", My.Settings.sMsg, MessageBoxButtons.OK, MessageBoxIcon.Information)
            End If
            dtgvD.Enabled = True
            CargaDatos(1)
            HabilitaBotones()
    ...
    Private Function GetResponsable() As String
        Try
            ...
            sSQL = "SELECT responsable FROM areas WHERE areaid='" & My.Settings.sAreaID & "'"
            Dim table As New DataTable()
            Dim adapter As New MySqlDataAdapter(sSQL, connection)
            adapter.Fill(table)
    ...
    Private Function GetArea() As String
        Try
            ...
            sSQL = "SELECT area FROM areas WHERE areaid='" & sAreadID & "'"
            Dim table As New DataTable()
            Dim adapter As New MySqlDataAdapter(sSQL, connection)
            adapter.Fill(table)
    ...
    Private Function BuscaTotales(ByVal nOpcion As Byte) As Integer
        Try
            Dim sSQl As String

            Select Case nOpcion
                Case 1
                    sSQl = "select count(folio) as total from peticiones "
                Case 2
                    sSQl = "select count(folio) as total from peticiones where estatus='NO ATENDIDO'"
                Case 3
                    sSQl = "select count(folio) as total from peticiones where estatus='EN TRAMITE'"
                Case 4
                    sSQl = "select count(folio) as total from peticiones where estatus='FINALIZADO'"
                Case 5
                    sSQl = "select count(folio) as total from peticiones where estatus='CANCELADO'"
            End Select
    ...
    Private Sub peticionesarea_KeyDown(sender As Object, e As KeyEventArgs) Handles MyBase.KeyDown
        Select Case e.KeyCode
